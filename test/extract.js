...
describe('extract', function () {
  it('should extract expression from Arazzo runtime expression', function () {
    ...
    assert.strictEqual(extract('{$inputs.username}'), '$inputs.username');
    assert.strictEqual(
      extract('{$workflows.foo.inputs.username}'),
      '$workflows.foo.inputs.username',
    );
    assert.strictEqual(
      extract('{$steps.someStepId.outputs.pets}'),
      '$steps.someStepId.outputs.pets',
    );
    assert.strictEqual(extract('{$outputs.bar}'), '$outputs.bar');
    assert.strictEqual(extract('{$workflows.foo.outputs.bar}'), '$workflows.foo.outputs.bar');
    assert.strictEqual(extract('{$components.parameters.foo}'), '$components.parameters.foo');
